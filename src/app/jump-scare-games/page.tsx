...
export default function JumpScareGamesPage() {
  const { t } = useTranslation();
  const [sortBy, setSortBy] = useState('rating');

  const genreKey = 'jump-scare';

  // 根据类型筛选游戏
  const filteredGames = horrorGames.filter(game =>
    game.genre.some(g => g.toLowerCase().includes('jump') || g.toLowerCase().includes('scare'))
  );
  ...
  const title = 'Jump Scare Horror Games';
  const description = 'Find the scariest jump scare games that will make you scream.';
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "itemListElement": [
              ...
              { "@type": "ListItem", "position": 2, "name": "Jump Scare Games", "item": "https://horrorgames.games/jump-scare-games" }
            ]
          })
        }}
      />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "FAQPage",
              "mainEntity": [
                { "@type": "Question", "name": "What are jump scare games?", "acceptedAnswer": { "@type": "Answer", "text": "Horror games designed around sudden scares and audio-visual shocks. Best played with headphones!" }},
                { "@type": "Question", "name": "Are jump scare games suitable for everyone?", "acceptedAnswer": { "@type": "Answer", "text": "They can be intense. Consider content warnings and player comfort before playing." }},
                { "@type": "Question", "name": "Which jump scare game should I start with?", "acceptedAnswer": { "@type": "Answer", "text": "Try popular picks near the top of our list for an accessible introduction." }}
              ]
            })
          }}
        />
