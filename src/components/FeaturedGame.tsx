...
import { Play, ExternalLink, Star, Calendar, User } from 'lucide-react';
...
export default function FeaturedGame({ game, onPlayClick }: FeaturedGameProps) {
  ...
  return (
    <motion.div
      className="relative bg-gray-900 rounded-2xl overflow-hidden shadow-2xl border border-gray-800 mb-8"
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {game.genre.map((genre) => (
                <span
                  key={genre}
                  className="px-3 py-1 bg-red-600/80 text-white rounded-full text-sm font-medium"
                >
                  {genre}
                </span>
              ))}
            ...
            <div className="flex space-x-4">
              {game.iframeUrl && (
                <motion.button
                  className="flex items-center space-x-2 bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-xl font-bold transition-all duration-200 shadow-lg"
              ...
              {game.steamUrl && (
                <motion.button
                  className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold transition-all duration-200 shadow-lg"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
